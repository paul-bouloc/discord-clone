...
export const updateServerName = async (req: Request, res: Response) => {
  ...
  if (!member || member.role !== MemberRole.ADMIN)
    throw new ForbiddenException(
      "You are not allowed to update the server name",
    );
...
export const updateServerBanner = async (req: Request, res: Response) => {
  ...
  if (!member || member.role !== MemberRole.ADMIN)
    throw new ForbiddenException(
      "You are not allowed to update the server banner",
    );
...
export const deleteServer = async (req: Request, res: Response) => {
  ...
  const member = await MemberService.findById(serverId, req.user!.id);
  if (!member || member.role !== MemberRole.ADMIN)
    throw new ForbiddenException("You are not allowed to delete this server");
